...
public class TestSetController {
    ...
    public Result<QueryResultDTO> queryBlacklist(@RequestBody QueryRequestParam params) {
        ...
        if (params.getWeight() == null) {
            return Result.error(400, "weight参数不能为空");
        }
        if (params.getEffectiveLambda() == null) {
            return Result.error(400, "effectiveLambda参数不能为空");
        }
        if (params.getLogPolyMod() == null) {
            return Result.error(400, "logPolyMod参数不能为空");
        }
        ...
        try {
            QueryResultDTO result = testSetService.queryBlacklist(
                    ...
                    params.getWeight(),           // 新增
                    params.getEffectiveLambda(),  // 新增
                    params.getLogPolyMod()        // 新增
            );
